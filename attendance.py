...
class Attendance:
    ...
    def __init__(self, root):
        ...
        # Create attendance table when initializing
        self.create_attendance_table()

        # This part is image labels setting start 
        # first header image  
        try:
            img = Image.open(r"C:\Users\user\OneDrive\Desktop\Python_Test_Projects\college_iimages\ee.png")
            img = img.resize((1366, 130), Image.LANCZOS)
            self.photoimg = ImageTk.PhotoImage(img)
            f_lb1 = Label(self.root, image=self.photoimg)
            f_lb1.place(x=0, y=0, width=1366, height=130)
        except:
            f_lb1 = Label(self.root, text="Attendance System", font=("verdana", 20, "bold"), bg="lightblue", fg="darkblue")
            f_lb1.place(x=0, y=0, width=1366, height=130)

        # background image 
        try:
            bg1 = Image.open(r"C:\Users\user\OneDrive\Desktop\Python_Test_Projects\college_iimages\sun.jpg")
        ...
        # title section
        title_lb1 = Label(bg_img, text="Welcome to Attendance Panel", font=("verdana", 30, "bold"), bg="white", fg="red")
        ...
        # Left Label Frame 
        left_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE, text="Student Details", font=("verdana", 12, "bold"), fg="red")
        left_frame.place(x=10, y=10, width=660, height=480)

        # ==================================Text boxes and Combo Boxes====================

        # Student id
        studentId_label = Label(left_frame, text="Std-ID:", font=("verdana", 12, "bold"), fg="red", bg="white")
        ...
        # Student Roll
        student_roll_label = Label(left_frame, text="Roll.No:", font=("verdana", 12, "bold"), fg="red", bg="white")
        ...
        # Student Name
        student_name_label = Label(left_frame, text="Std-Name:", font=("verdana", 12, "bold"), fg="red", bg="white")
        ...
        # time
        time_label = Label(left_frame, text="Time:", font=("verdana", 12, "bold"), fg="red", bg="white")
        ...
        # Date 
        date_label = Label(left_frame, text="Date:", font=("verdana", 12, "bold"), fg="red", bg="white")
        ...
        # Attendance
        student_attend_label = Label(left_frame, text="Attend-status:", font=("verdana", 12, "bold"), fg="red", bg="white")
        ...
        # Import button
        save_btn = Button(btn_frame, command=self.importCsv, text="Import CSV", width=12, font=("verdana", 12, "bold"), fg="white", bg="red")
        save_btn.grid(row=0, column=0, padx=6, pady=10, sticky=W)

        # Export button
        update_btn = Button(btn_frame, command=self.exportCsv, text="Export CSV", width=12, font=("verdana", 12, "bold"), fg="white", bg="red")
        update_btn.grid(row=0, column=1, padx=6, pady=8, sticky=W)

        # Update button
        del_btn = Button(btn_frame, command=self.action, text="Update", width=12, font=("verdana", 12, "bold"), fg="white", bg="red")
        del_btn.grid(row=0, column=2, padx=6, pady=10, sticky=W)

        # reset button
        reset_btn = Button(btn_frame, command=self.reset_data, text="Reset", width=12, font=("verdana", 12, "bold"), fg="white", bg="red")
        reset_btn.grid(row=0, column=3, padx=6, pady=10, sticky=W)

        # Right section=======================================================

        # Right Label Frame 
        right_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE, text="Student Details", font=("verdana", 12, "bold"), fg="red")
        ...
        # Update button
        update_btn = Button(right_frame, command=self.update_data, text="Update", width=12, font=("verdana", 12, "bold"), fg="white", bg="red")
        update_btn.grid(row=0, column=1, padx=6, pady=10, sticky=W)
        
        # Delete button
        del_btn = Button(right_frame, command=self.delete_data, text="Delete", width=12, font=("verdana", 12, "bold"), fg="white", bg="red")
        del_btn.grid(row=0, column=2, padx=6, pady=10, sticky=W)
    ...
    def create_attendance_table(self):
        ...
        try:
            ...
            if conn:
                ...
                if count == 0:
                    insert_query = """
                    INSERT INTO attendance (std_id, std_roll_no, std_name, std_time, std_date, std_attendance) 
                    VALUES (%s, %s, %s, %s, %s, %s)
                    """
                    sample_data = [
                        ('1', '2024001', 'John Doe', '10:30:00', '15/12/2024', 'Present'),
                        ('2', '2024002', 'Jane Smith', '10:32:00', '15/12/2024', 'Present'),
                        ('3', '2024003', 'Mike Johnson', '10:35:00', '15/12/2024', 'Absent')
                    ]
            ...
            try:
                ...
                if Update:
                    ...
                    if conn:
                        ...
                        mycursor.execute("UPDATE attendance SET std_roll_no=%s, std_name=%s, std_time=%s, std_date=%s, std_attendance=%s WHERE std_id=%s", (
                            self.var_roll.get(),
                            self.var_name.get(),
                            self.var_time.get(),
                            self.var_date.get(),
    ...
    def exportCsv(self):
        try:
            ...
            if fln:
                ...
                messagebox.showinfo("Successfully", "Export Data Successfully!")
        except Exception as es:
            messagebox.showerror("Error", f"Due to: {str(es)}", parent=self.root)
    ...
    def action(self):
        if self.var_id.get() == "" or self.var_roll.get() == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                ...
                if conn:
                    mycursor = conn.cursor()
                    mycursor.execute("INSERT INTO attendance (std_id, std_roll_no, std_name, std_time, std_date, std_attendance) VALUES (%s, %s, %s, %s, %s, %s)", (
                        self.var_id.get(),
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_time.get(),
